...
func ThemeSwitcher(theme string) ProbabilityData {
	...
	case "Алгоритм Дейкстры":
		return ProbabilityData{
			...
			TaskImage: "asserts/images/пример_алгоритм_Дейкстры.png",
			Hint: "Начните с вершины A — она получает метку 0, остальные — бесконечность.\n"+
			"На каждом шаге выбирайте необработанную вершину с наименьшей меткой.\n"+
			"Обновите расстояния до её соседей, если найденный путь через текущую вершину короче.\n"+
			...
			"\n"+
			"Из B:\n"+
			"B → C: уже 2, через B было бы 4 + 5 = 9 → не меньше\n"+
			"B → D: 4 + 4 = 8 → не меньше 6\n"+
			"Ничего не обновляется\n"+
			"Обрабатываем D\n"+
			"\n"+
			"Из D:\n"+
			"D → F: 6 + 1 = 7 → уже есть 7, не обновляется\n"+
			"Обрабатываем F\n"+
			"\n"+
			"Финальные метки (кратчайшие расстояния от A):\n"+
			"A: 0\n"+
			"B: 4\n"+
			"C: 2\n"+
			"D: 6\n"+
	...
	case "Решатор задач":
		return ProbabilityData{
			Title:  "Решатор задач",
			Theory: "Практикуйтесь в решении задач по всем темам дискретной математики. Введите ответ и проверьте его правильность. После 3 неверных попыток можно посмотреть решение.",
		}
